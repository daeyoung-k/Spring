...
public class PageVO {
	...
	//PageVO가 생성될때cri, total반드시 전달 되야하기 때문에 한개로 제한
	public PageVO(Criteria cri, int total) {
		...
		//진짜 끝번호 계산
		//데이터가 60개 있다고 가정할때, pageNum 5번 누르면 끝페이지번호 6번
		//데이터가 112개 있다고 가정할때, pageNum 11번 누르면 끝페이지번호 12번
		int realEnd = (int)Math.ceil( this.total / (double)this.amount);
		//특정조건에서 endPage를 변경
		//123개 게시글이 존재할때,
		//11번 클릭을 할때: endPage페이지 공식 -> 20. realEnd -> 13
		//2번을 클릭할때: endPage페이지 공식 -> 10, realEnd -> 13
